...
class QuantumBertForSequenceClassification(BertPreTrainedModel):

    def __init__(self, config,num_labels=2):
        super(QuantumBertForSequenceClassification, self).__init__(config)
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
        ...
        outputs = self.bert(input_ids,token_type_ids, attention_mask, output_all_encoded_layers=True, output_att=True,position_ids=position_ids)
        # encoded_layers, layer_atts, pooled_output
        encoded_layers = outputs[0]
        last_layer = outputs[0][-1]
        if False:
            pooled_output = torch.mean(last_layer,-2)
            pooled_output = self.dropout(pooled_output)
        else:
            batch_size, seq_length, dim = last_layer.size()
            rho = torch.bmm(last_layer.unsqueeze(-1).view(-1, dim, 1), last_layer.unsqueeze(-2).view(-1, 1, dim)).view(batch_size,seq_length,dim,dim).sum(-3)
            diag = torch.diagonal(rho, offset=0, dim1=-1, dim2=-2)
            row = rho.max(-1)[0]
            col = rho.max(-2)[0]
            pooled_output = torch.cat([diag,row,col],-1)
        logits = self.classifier(pooled_output)
